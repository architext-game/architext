...
def event(
        ...
        Out: Type[ResponseModel[Any]], # The output pydantic model
        In: Optional[Type[BaseModel]] = None, # The input pydantic model to validate request JSON
    ):
    """
    Decorator to register a Socket.IO event handler with automatic validation.

    - Given a socketio server (`sio`) and a socketio event name (`on`),
    - Validates input against a Pydantic model (`In`).
    - Wraps the handler's return value into the response model (`Out`) 
      with standardized success/error/data fields.
    - Registers the endpoint for automatic SDK generation.
    """
