    ...
    def process_reform_value(self, message):
        ...
        if message:
            if self.option_number == 0:  # edit name
                ...
                if entities.Room.objects(alias=message, world_state=self.session.user.room.world_state):
                    edited_properties['destination_room_id'] = entities.Room.objects.get(alias=message, world_state=self.session.user.room.world_state).id
                else:
            ...
            try:
                if self.item_to_edit is not None:
                    self.session.send_to_client(str(edited_properties))
                    services.update_item(self.session, self.item_to_edit.id, **edited_properties)
                if self.exit_to_edit is not None:
                    services.update_exit(self.session, self.exit_to_edit.id, **edited_properties)
